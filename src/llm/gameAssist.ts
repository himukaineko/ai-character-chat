...
export async function requestGameAssist(
  ...
  const prompt = [
    ...
    "- descriptionには、上がる条件と下がる条件の両方を必ず具体的に書く" +
      "(例: 「ユーザーに優しくされると上がる。冷たくされたり約束を破られると下がる」)。" +
      "上がる条件しかないステータスは作らない(下がるリスクがないとゲームとして緊張感が出ないため)",
    "- initialは0〜maxの範囲で、物語の始まりにふさわしい値にする(全くのゼロからより、少し余地がある値が望ましいことが多い)",
    "- min/maxは0〜100を基本にしつつ、ユーザーのヒントに合わせて調整してよい",
    "",
    "## 展開ルール(rulesPrompt)の書き方",
    "次の3ブロック構成で、この順に書いてください(見出しは【 】)。",
    "1.【ステータス名】ブロック(ステータス1つにつき1ブロック):",
    "   1行目に、そのステータスが何を表すか(例: 関係の深さ / 執着の強さ)と「キャラごとに個別判定」を明記する。",
    "   続けて数値帯ごとの変化を1行ずつ書く(例: 「0〜20: よそよそしく業務的」「81〜100: 告白・秘めた過去の解禁」)。",
    "2.【組み合わせ】ブロック(ステータスが2つ以上あるとき必須):",
    "   ステータス同士が影響し合うルールを書く(例: 「好感度が低い状態で秘密を探ろうとすると好感度がさらに下がる」" +
      "「信頼度が低いのに依存度が高い: 心を開いていないのに手放せない歪んだ状態。監視・詮索として現れる」)。",
    "3.【キャラ別】ブロック(参加キャラクターの情報があるとき):",
    "   キャラ名ごとに、数値が高い/低いときの固有の反応を短く書く。性格に応じて同じ数値帯でも方向を変える" +
      "(例: 依存的・執着の強い性格のキャラは数値が高いと病む・束縛する方向へ、素直な性格のキャラは優しくなる方向へ)。" +
      "夢・悩み・秘密があるキャラは、特定の数値帯でそれが明かされる・破滅につながる、といった展開に活かす。",
    "禁止事項: 複数のステータスをまとめて1つの数値帯表にする「◯◯・△△共通のルール」は書かない。" +
      "数値帯表は必ずステータス1つにつき1つ(片方だけ高い状態のとき、矛盾した指示になってしまうため)。",
    "文量の方針: 数値帯の一般的な状態説明は1行で短く。キャラ固有の情報(秘密・過去・関係)を優先して具体的に書く。" +
      "AIが会話生成時にそのまま参照する文章になるため、全体を簡潔にまとめる。",
    "",
    "## 出力形式",
