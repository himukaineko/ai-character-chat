...
const NARRATION_INSTRUCTIONS: Record<NarrationLevel, string> = {
  ...
  light:
    "セリフのテンポには変化をつけてください。毎回セリフと行動描写をセットにする必要はありません。セリフだけが続く場面があってもよいですし、逆に行動描写だけで間を持たせる一言があってもよいです。テンポの良い場面では描写を省き、感情を掘り下げたい場面でだけ描写を入れる、というように使い分けてください。加えて、場面が変わるとき・間を置きたいときには、type: \"narration\" の短い地の文をときどき挟んでください(novelほど頻繁でなくてよく、セリフ中心の会話にたまに一言添える程度でよい。一切出さないのは避けてください)。",
