  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <div className="mt-8">
        ...
        <AccordionSection title="ゲームモード">
          <BulletList>
            <Bullet>
              ルーム設定の<Tag>ゲームモード</Tag>をONにすると、恋愛シミュレーションのような遊び方ができます。キャラごとに好感度などのステータスを複数定義できます。
            </Bullet>
            <Bullet>
              会話の内容に応じて、AIが会話生成と同じタイミングでステータスを自動的に上下させます(変動が無いのが普通で、発言や出来事に見合った時だけ動きます)。
            </Bullet>
            <Bullet>
            ...
            <Bullet>
              現在値は保存されている数値ではなく、これまでの変動の合計から計算されるため、<Tag>元に戻す</Tag>や<Tag>ここまで戻る</Tag>を使うと、取り消した会話ぶんの変動も自動的に元に戻ります。
            </Bullet>
          </BulletList>
        </AccordionSection>
        ...
        <AccordionSection title="バックアップと共有">
          <BulletList>
            ...
            <Bullet>
              ルーム1つだけを書き出したいときは、ルーム画面の<Tag>ログ管理</Tag>から<Tag>ルームをエクスポート</Tag>を使います。「ログ込み」を選ぶと会話の続きから遊べる状態のまま、「設定のみ」を選ぶと会話ログを含まない共有向けのファイルになります。参加キャラや紐づくワールドも一緒に書き出され、設定画面の<Tag>インポート</Tag>から読み込めます。
            </Bullet>
